      ...
      <div class="site-wrapper-inner">
        <div class="cover-container">
            ...
            <?php
                ...
                if(!$conexion){
                    echo "Conexion fallida :(<br>Revisa que los datos de conexión a la DB en unirseClan.php sean correctos<br>";
                }
                else
                {
                    ...
                    if ($ver[0] == 'v') {
                        ...
                        $query = "SELECT modificar_exp_clan ('".$nombre_clan."','".$exp."')";
                        $result = pg_query($conexion,$query);           
                        if ($result) {
                            //Obtener EXP del clan
                            $query = "SELECT obtener_exp_clan ('".$nombre_clan."')";
                            $result = pg_query($conexion,$query);   
                            $fila = pg_fetch_row($result);
                            $exp_clan = $fila[0];
                            $_SESSION["exp_clan"] = $exp_clan;
                        }
                        pg_close($conexion);
                        ...
                        <h3 class=\"alert alert-danger\">
                            <strong>Error! </strong>El clan especificado no existe!
                        </h3>
